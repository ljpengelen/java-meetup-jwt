...
The other is stored in a secure, HTTP-only cookie, which is only accessible for the back end (when it is sent along with the next request).
...
For each request to a protected endpoint, the back end checks whether the token in the header and the cookie match.
When they match, the request is granted.
